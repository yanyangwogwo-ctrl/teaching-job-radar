...
版本：2026-09-17；由使用者最初 13 個來源擴展至 33 個官方招聘來源。
...
完整官方 URL、host allowlist、讀取方式及說明以 `config/sites.yaml` 為準。33 個來源已加入每日檢查，其中未能驗證廣告格式的來源只核對連線並明示未完成；接入不等於每輪成功，實際狀態以雲端結果為準。來源成功要求清單、分頁和預期詳情均完整；詳情失效也要顯示 partial。

嶺南使用官方頁面提供的匿名工作階段，讀取公開搜尋及廣告 API。只接受指定院校、匿名使用者與 API 主機；token 不寫入資料或日誌，不加入 session 預設標頭，也不發往 robots.txt。清單日期按已確認的 en-US M/D/YYYY 解析；不用內部管理日期或 description 佔位文字。每則廣告遵守 10 秒間隔，首次完整檢索約需 17 分鐘。
...
- 搜尋頁提供 `presetEnabled` 開關；「預設條件」分頁編輯 `subjectKeywords`、`roleKeywords`、`presetExcludeKeywords` 及 `partTimeOnly`。科目任一 OR AND 職位名稱任一 OR；科目比對職位名、部門與內文（清理哲學學位、一般招聘頁尾及非教學的批判思考能力要求），職位名稱只比對標題。預設排除字詞以逗號（中英文皆可）或換行分隔，保留詞組空格，按搜尋職位名、部門、科目與完整內文的相同規則比對，任一命中即排除；原始預設及缺少此欄的舊常用搜尋均留空。關閉開關會暫停四項預設限制，保留內容；手動 query／搜尋頁排除／院校／日期／狀態仍適用。
- 舊分組常用搜尋原樣保留，包括空欄。合併版本則以常見職位詞分組，其餘作科目；非空但缺組時補上原始預設，空合併欄保留為兩組皆空。原有詞串／匹配方式保留於 legacyMergedPreset 並在分頁提示核對，不覆寫原 localStorage 記錄。統計及來源警告維持精簡排版；正式通知評分規則獨立。
