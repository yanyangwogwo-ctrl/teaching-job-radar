...
完整官方 URL、host allowlist、讀取方式及說明以 `config/sites.yaml` 為準。13 個來源均已接入每日檢索；接入不等於每輪成功，實際狀態以雲端結果為準。來源成功要求清單、分頁和預期詳情均完整；詳情失效也要顯示 partial。
