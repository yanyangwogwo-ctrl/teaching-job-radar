...
- 搜尋頁只保留預設條件開關；「預設條件」分頁可編輯合併後的科目／職位關鍵字、AND／OR 及兼職限制。所有詞搜尋職位名、部門、科目和內文，預設任一 OR；例如只有 lecturer 也符合，結果會比舊有兩組 AND 較多。清空合併欄位取消預設關鍵字限制。
- 關閉預設條件會暫停其關鍵字及兼職限制，保留院校、日期、手動搜尋及排除條件；再開啟沿用修改後內容。儲存常用搜尋包含開關及預設內容，舊版兩欄會自動合併。
- 統計數字縮到分頁列右側，來源警告移到搜尋結果上方；小螢幕會換行，搜尋內容毋須再讓位給大型摘要卡片。
