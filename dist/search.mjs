export function normalize(value) {
  return String(value ?? '').normalize('NFKC').toLowerCase()
    ...
    .replace(/\bfull[\s-]*time\b/g, 'fulltime').replace(/\b(lecturer|tutor|instructor|teacher)s\b/g, '$1')
    .replace(/\s+/g, ' ').trim();
}
...
export function filterJobs(jobs, criteria, today) {
  ...
  const result = jobs.filter(job => {
    ...
    if (criteria.relevantOnly && !job.matches) return false;
